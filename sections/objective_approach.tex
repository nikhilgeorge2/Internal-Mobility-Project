...
\begin{figure}
    ...
    \begin{tikzpicture}[node distance=1.2cm]
        ...
        \tikzstyle{data} = [rectangle, minimum width=2.5cm, minimum height=1cm, text centered, draw=black, fill=green!10]
        \tikzstyle{arrow} = [thick,->,>=stealth]
        \tikzstyle{dotted} = [thick,->,>=stealth,dotted]
    
        % Main pipeline nodes
        \node (embeddings) [process] {Pre-trained Embeddings};
        \node (reduction) [process, right=2cm of embeddings] {Dimension Reduction};
        \node (metric) [process, right=2cm of reduction] {Distance Metric Learning};
        \node (tuning) [process, right=2cm of metric] {Parameter Tuning};
        
        % Validation node
        \node (validation) [data, below=0.8cm of tuning] {Validation};
        
        % Selection data node
        \node (selection) [data, below=0.8cm of metric] {Selection Data};
        ...
        \draw [dotted] (tuning) to[bend right] node[midway, below] {Adjust} (reduction);
        \draw [dotted] (selection) -- (metric) node[midway, right] {Guide};
    
        % Add annotations
        \node [text width=2cm, align=center, font=\small] at (embeddings.north) [above=0.2cm] {Text to Vectors};
        \node [text width=2cm, align=center, font=\small] at (reduction.north) [above=0.2cm] {Focus Signal};
        \node [text width=2cm, align=center, font=\small] at (metric.north) [above=0.2cm] {Learn Patterns};
    
    \end{tikzpicture}
    \caption{Machine Learning Pipeline. Job posting text is transformed into vectors, reduced to relevant dimensions, and used to learn a distance metric guided by selection patterns. Parameter tuning ensures optimal performance through iterative adjustment.}
