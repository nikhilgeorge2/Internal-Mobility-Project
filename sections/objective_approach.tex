...
\begin{figure}
    ...
    % Enhanced styles for boxes and arrows
    \tikzstyle{box} = [rectangle, draw, text width=2.5cm, text centered, minimum height=1.5cm, rounded corners=3pt, fill=gray!5]
    \tikzstyle{arrow} = [thick,->,>=stealth]
    \tikzstyle{dotted arrow} = [thick,->,>=stealth,dotted]
    \tikzstyle{note} = [text width=2.5cm, font=\small, align=center]
    
    % Job postings box (new)
    \node[box] (postings) {Job Postings};
    
    % Main pipeline nodes
    \node[box, right=of postings] (embeddings) {Pre-trained Embeddings};
    ...
    \node[box, below=0.8cm of selection] (validation) {Validation};
    
    % Selection data box (new)
    \node[box, dashed, below=0.8cm of metric] (seldata) {Selection Data};
    
    % Annotations (new)
    \node[note, above=0.3cm of embeddings] {Transform text to vectors};
    \node[note, above=0.3cm of reduction] {Focus on relevant dimensions};
    \node[note, above=0.3cm of metric] {Learn from patterns};
    
    % Main flow arrows
    \draw[arrow] (postings) -- (embeddings);
    ...
    \draw[dotted arrow] (seldata) -- (metric) node[midway, right] {Guide learning};
    
    \end{tikzpicture}
    \caption{Machine Learning Pipeline Overview. Job posting text is transformed into vector representations, reduced to relevant dimensions, and used to learn a distance metric guided by selection patterns. Parameter tuning ensures optimal performance, with validation confirming the informativeness of the learned distances.}
